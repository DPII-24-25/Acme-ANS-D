...
@Repository
public interface FligthRepository extends AbstractRepository {

	@Query("select min(l.scheduleDeparture) from Leg l where l.fligth.id =:id")
	Date getScheduleDeparture(int id);

	@Query("select max(l.scheduleArrival) from Leg l where l.fligth.id =:id")
